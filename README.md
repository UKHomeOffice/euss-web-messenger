...
Home Office service for EUSS web messenger. This service provides a chat bot style services for public use.
...
> Note: Non-prod values can be found in Keybase (`euss-wm-env.json`), a senior developer on the team can help you to find this if you're unfamiliar with Keybase.
...
```bash
docker build -t euss-web-messenger:local .
```
...
> NOTE: the file doesn't have to be named `.env` it's just to pass the environment variables to the docker environment. Again the non-prod values can be found in keybase (`euss-wm-env.json`).

3. Run the container with the env file and port mapping:

```bash
docker run --rm --env-file .env -p 8000:80 euss-web-messenger:local
```
